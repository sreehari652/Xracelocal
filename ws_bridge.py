...
class LapScore:
    def __init__(self, car_id, car_name, lap_number):
        ...
        self.top_speed_kmh = 0.0   # max tag speed recorded during this lap
...
class ScoringEngine:
    ...
    def close_lap(self, cid, raw, top_speed_kmh=0.0):
        ...
        lap.top_speed_kmh = round(float(top_speed_kmh), 2)
        self._history[cid].append(lap)
        msg = f"📊 LAP | {lap.car_name} Lap {lap.lap_number} raw={raw:.2f}s ELP={lap.elp:.2f}s  top={lap.top_speed_kmh:.1f}km/h"
...
class LapEngine:
    ...
    def _process_crossing(self, now):
        ...
        # Pull the tag's per-lap max speed (cm/s → km/h) then reset so
        # the next lap accumulates from zero.
        tag_state = tags.get(self.car_id)
        top_kmh = round((tag_state.max_speed_ms * 0.036), 2) if tag_state else 0.0
        if tag_state:
