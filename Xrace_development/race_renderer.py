...
class RaceRenderer:
    ...
    def draw_start_line(self):
        """Draw the start/finish line(s)"""
        if not SHOW_START_LINE:
            return

        if USE_SAME_LINE_FOR_START_FINISH:
            # Traditional racing: same line for start and finish
            ...
            if START_LINE_ORIENTATION == 'vertical':
                ...
                # Draw dual-colored line (green top half, red bottom half)
                mid_y = (y1_pixel + y2_pixel) // 2

                # Green half (START)
                pygame.draw.line(self.screen, START_LINE_COLOR,
                                 (x_pixel, y1_pixel), (x_pixel, mid_y),
                                 START_LINE_WIDTH)

                # Red half (FINISH)
                pygame.draw.line(self.screen, FINISH_LINE_COLOR,
                                 (x_pixel, mid_y), (x_pixel, y2_pixel),
                                 FINISH_LINE_WIDTH)

                # Draw labels
                start_label = self.font_race_title.render("START", True, START_LINE_COLOR)
                finish_label = self.font_race_title.render("FINISH", True, FINISH_LINE_COLOR)
                self.screen.blit(start_label, [x_pixel + 10, y1_pixel + 10])
                self.screen.blit(finish_label, [x_pixel + 10, y2_pixel - 30])

                # Draw checkered pattern in middle
                self._draw_checkered_pattern(x_pixel, mid_y - 50, x_pixel, mid_y + 50)

            else:  # horizontal
                x1_pixel, y_pixel = self.base_renderer.cm_to_pixels(START_LINE_X1, START_LINE_Y)
                x2_pixel, _ = self.base_renderer.cm_to_pixels(FINISH_LINE_X2, START_LINE_Y)

                # Draw dual-colored line
                mid_x = (x1_pixel + x2_pixel) // 2

                pygame.draw.line(self.screen, START_LINE_COLOR,
                                 (x1_pixel, y_pixel), (mid_x, y_pixel),
                                 START_LINE_WIDTH)

                pygame.draw.line(self.screen, FINISH_LINE_COLOR,
                                 (mid_x, y_pixel), (x2_pixel, y_pixel),
                                 FINISH_LINE_WIDTH)

                start_label = self.font_race_title.render("START", True, START_LINE_COLOR)
                finish_label = self.font_race_title.render("FINISH", True, FINISH_LINE_COLOR)
                self.screen.blit(start_label, [x1_pixel + 10, y_pixel - 25])
                self.screen.blit(finish_label, [x2_pixel - 80, y_pixel - 25])

                self._draw_checkered_pattern(mid_x - 50, y_pixel, mid_x + 50, y_pixel)
            ...
            if START_LINE_ORIENTATION == 'vertical':
                x_pixel, y1_pixel = self.base_renderer.cm_to_pixels(FINISH_LINE_X, FINISH_LINE_Y1)
                _, y2_pixel = self.base_renderer.cm_to_pixels(FINISH_LINE_X, FINISH_LINE_Y2)
    ...
    def _draw_checkered_pattern(self, x1, y1, x2, y2):
        ...
        # Checkered pattern with better contrast on white background
        square_size = 10
        ...
        if x1 == x2:  # Vertical line
            ...
            for i in range(num_squares):
                ...
                pygame.draw.rect(self.screen, GRAY,
                                 (x1 - square_size // 2, y, square_size, square_size), 1)
            ...
            for i in range(num_squares):
                ...
                pygame.draw.rect(self.screen, GRAY,
                                 (x, y1 - square_size // 2, square_size, square_size), 1)
    ...
    def draw_leaderboard(self, race_manager, collision_detector):
        ...
        for idx, car_info in enumerate(leaderboard):
            ...
            # Color based on position (darker for white background)
            if position == 1:
                color = [218, 165, 32]  # Darker gold
            elif position == 2:
                color = [128, 128, 128]  # Darker silver
            elif position == 3:
                color = [165, 88, 33]  # Darker bronze
            else:
                color = BLACK
    ...
    def render_race_frame(self, anchors, tags, race_manager, speed_manager,
        ...
        # Draw base elements (grid, anchors, etc.)
