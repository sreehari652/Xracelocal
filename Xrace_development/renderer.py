...
class Renderer:
    ...
    def draw_anchor(self, anchor):
        ...
        # Draw label (darker color for white background)
        temp_str = f"{anchor.name} ({anchor.x},{anchor.y})"
        surf = self.font_info.render(temp_str, True, WHITE)
        self.screen.blit(surf, [pixel_x + 12, pixel_y - 8])

        # Draw anchor circle
        pygame.draw.circle(self.screen, BLACK, [pixel_x, pixel_y], ANCHOR_RADIUS, 0)
        pygame.draw.circle(self.screen, GRAY, [pixel_x, pixel_y], ANCHOR_RADIUS + 2, 2)
    ...
    def draw_tag(self, tag):
        ...
        pygame.draw.circle(self.screen, BLACK, [pixel_x, pixel_y], TAG_RADIUS + 2, 2)
    ...
    def draw_info_panel(self, tags, udp_receiver):
        ...
        # Title (darker for white background)
